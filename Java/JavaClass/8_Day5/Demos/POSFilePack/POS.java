...
class SaleList
{
	...
	public void SaveToFile()
	{
		try
		{
		ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream("F:\\SaleListData.txt"));
	    out.writeObject(a);
	    out.close();
		}
		catch(Exception e)
	...
	public void RetrieveFromFile()
	{
		...
		try
		{
			 ...
			 ObjectInputStream in = new ObjectInputStream(new FileInputStream("F:\\SaleListData.txt"));
			 ArrayList<SaleItem> r;
		      r =(ArrayList<SaleItem>) in.readObject();
		       for(int i=0;i<r.size();i++)
				{
					st = r.get(i);
					a.add(st);
					t = st.getT();
					System.out.println(t.getDesc()+"\t"+t.getRate()+"\t"+st.getQuantity());
				}
		       in.close();
